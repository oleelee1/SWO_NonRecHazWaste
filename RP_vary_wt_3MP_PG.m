    ...
    for P1 = 1:length(wt)
        ...
        Dens = Vmat(2);
            ...
            %% start wt iteration
                    ...
                    Massflow_wat = mass_basis - Massflow_3MP - Massflow_Ox-Massflow_PG; %OR MASS FLOW PG
           ...
           CompWork_HP = CompWork_KW * 1.341;
            ...
            %% Attempt at integrating the 1/k expression to calculate residence time
                    ...
                    Xa = 0:0.01:X(x);
                    c = 1-Xa;
                    d = (T1+273) + ((wt(P1).*Xa.*Delhr)./(CpAv)) + ((Xa.*Massflow_PG*DelhrPG)/(mass_basis*CpAv));
                    e = 1./d;
                    f = ((-a).*e)+b;
                    g = exp(f);
                    h = c.*g;
                    ...
                    T2(i) = (T1+273) + ((wt(P1)*X(x)*Delhr)/(CpAv)) + ((X(x).*Massflow_PG*DelhrPG)/(mass_basis*CpAv));
                    T2(i) = T2(i)-273;
...
savefig(1,'wt_vs_T2_3MP_PG_steam')
savefig(2,'wt_vs_poweroutwat_3MP_PG_steam')
savefig(3,'wt_vs_powerout_3MP_PG_steam')
savefig(4,'wt_vs_restime_3MP_PG_steam')
save('wt_3MP_PG_steam')