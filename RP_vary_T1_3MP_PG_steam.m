...
Massflow_3MP_P1 = ones(length(T1),1);
...
Massflow_3MP_P1_X = ones(length(T1),length(X));
    ...
    for P1 = 1:length(T1)
        ...
        Dens = Vmat(2);
            ...
            %% start wt iteration
                    ...
                    Massflow_3MP = mass_basis*wt; %kg/hr
                    ...
                    Massflow_wat = mass_basis - Massflow_3MP - Massflow_Ox - Massflow_PG;
           ...
           CompWork_HP = CompWork_KW * 1.341;
            ...
            %% Attempt at integrating the 1/k expression to calculate residence time
                    ...
                    Xa = 0:0.01:X(x);
                    c = 1-Xa;
                    d = (T1(P1)+273) + ((wt.*Xa.*Delhr)./(CpAv)) + ((Xa.*Massflow_PG*DelhrPG)/(mass_basis*CpAv));
                    e = 1./d;
                    f = ((-a).*e)+b;
                    g = exp(f);
                    h = c.*g;
                    ...
                    T2(i) = (T1(P1)+273) + ((wt*X(x)*Delhr)/(CpAv))+ ((X(x).*Massflow_PG*DelhrPG)/(mass_basis*CpAv));
                    T2(i) = T2(i)-273;
...
%% Results
            ...
            T1_P1(P1) = T1(P1);
            CpAv_P1(P1) = CpAv;
            Massflow_3MP_P1(P1) = Massflow_3MP;
            C0f_P1(P1) = C0f;
            mS_P1(P1) = mS;
            RT_P1(P1) = RT;
    ...
    RT_P1_X(:,x) = RT_P1;
    C0f_P1_X(:,x) = C0f_P1;
    Massflow_3MP_P1_X(:,x) = Massflow_3MP_P1; % rows - T2, columns - X
    mS_P1_X(:,x) = mS_P1;
...
savefig(1,'T1_vs_T2_3MP_PG_steam')
savefig(2,'T1_vs_poweroutwat_3MP_PG_steam')
savefig(3,'T1_vs_powerout_3MP_PG_steam')
savefig(4,'T1_vs_restime_3MP_PG_steam')
save('T1_3MP_PG_steam')