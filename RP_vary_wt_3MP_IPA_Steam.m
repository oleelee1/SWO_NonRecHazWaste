...
Massflow_3MP_wt = ones(length(wt),1);
...
Massflow_3MP_wt_X = ones(length(wt),length(X));
    ...
    for P1 = 1:length(wt)
        ...
        Dens = Vmat(2);
            ...
            %% start wt iteration
                    ...
                    Massflow_3MP = mass_basis*wt(P1); %kg/hr
                    Molflow_Wat = mass_basis*(1-wt(P1))/Mrwat; %kmol/hr
                    Molflow_3MP = Massflow_3MP/Mr3MP; %kmol/hr
                    Molflow_Ox = Molflow_3MP*SR; % kmol/hr
                    ...
                    Massflow_wat = mass_basis - Massflow_3MP - Massflow_Ox-Massflow_IPA; %OR MASS FLOW PG
                    ...
                    C = 6; H = 7; O = 0;
                    Delhr = DelHR_mass(C,H,O,Mr3MP); % Kj/kg3MP
                    DelhrIPA = DelHR_mass(3,8,1,MrIPA); %KJ/kg
                    DelhrPG = DelHR_mass(3,8,2,MrPG);
                    
                    %kg3MP = TotalEnergy/(Delhr*X(x)); %kg3MP/hr
                    %wt(i) = kg3MP/mass_basis(mb); %kg3MP/kgfeed
                    kg3MP = Massflow_3MP;

                    kgCOD = (Massflow_3MP/Mr3MP)*MROx*32; % kgOxygen/hr
                    wtCOD = kgCOD/mass_basis; % kgCOD/kgFeed

                    C0kmolkg = wt(P1)/Mr3MP; % kmol/kgfeed
                    C0kmolvol = C0kmolkg*mass_basis/V; %kmol/m3
                    C0molvol = C0kmolvol*1000; %mol/m3
                    CODmolvol = (C0molvol)*MROx*32;

                    C0gvol = C0molvol*Mr3MP; %g/m3
           ...
           PumpWork_KW = specwork * (Massflow_3MP+Massflow_wat)/(3600*1000); %KJ/s or KW
           ...
           CompWork_HP = CompWork_KW * 1.341;
            ...
            %% Attempt at integrating the 1/k expression to calculate residence time
                    ...
                    Xa = 0:0.01:X(x);
                    c = 1-Xa;
                    d = (T1+273) + ((wt(P1).*Xa.*Delhr)./(CpAv)) + ((Xa.*Massflow_IPA*DelhrIPA)/(mass_basis*CpAv));
                    e = 1./d;
                    f = ((-a).*e)+b;
                    g = exp(f);
                    h = c.*g;
                    ...
                    T2(i) = (T1+273) + ((wt(P1)*X(x)*Delhr)/(CpAv)) + ((X(x).*Massflow_IPA*DelhrIPA)/(mass_basis*CpAv));
                    T2(i) = T2(i)-273;
...
power_organic = (Power*3600)/Massflow_3MP;
...
%% Results
            ...
            T2_wt(P1) = T2f;
            CpAv_wt(P1) = CpAv;
            Massflow_3MP_wt(P1) = Massflow_3MP;
            C0f_wt(P1) = C0f;
            mS_wt(P1) = mS;
            RT_wt(P1) = RT;
    ...
    RT_wt_X(:,x) = RT_wt;
    C0f_wt_X(:,x) = C0f_wt;
    Massflow_3MP_wt_X(:,x) = Massflow_3MP_wt; % rows - T2, columns - X
    mS_wt_X(:,x) = mS_wt;
...
savefig(1,'wt_vs_T2_3MP_IPA_steam')
savefig(2,'wt_vs_poweroutwat_3MP_IPA_steam')
savefig(3,'wt_vs_powerout_3MP_IPA_steam')
savefig(4,'wt_vs_restime_3MP_IPA_steam')
save('wt_3MP_IPA_steam')